...
BNB_IN(1) vec2 var_body_mask_uv;
...
void main()
{
    vec4 body_color = vec4(0.0, 0.25, 1.0, 1.);
    vec3 body =  BNB_TEXTURE_2D(BNB_SAMPLER_2D(s_bg), var_uv).xyz;
    const float threshold = 0.2;
    float mask = max((textureBicubic(BNB_PASS_SAMPLER_ARGUMENT(s_segmentation_mask),var_body_mask_uv).x - threshold) / (1.0 - threshold), 0.0);
    bnb_FragColor = vec4(blendColor(body, body_color.xyz, mask * body_color.w), 1.0);
}
