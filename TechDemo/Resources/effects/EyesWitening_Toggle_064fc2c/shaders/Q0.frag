...
BNB_DECLARE_SAMPLER_2D(2, 3, glfx_L_EYE_CORNEOSCLERA_MASK);
...
{   
	...
	float factor = blur(BNB_PASS_SAMPLER_ARGUMENT(glfx_L_EYE_CORNEOSCLERA_MASK)).x;
	vec4 c = whitening(bg, factor *eyesWhiteningCoeff, BNB_PASS_SAMPLER_ARGUMENT(lookupTexEyes));
	// c.xyz *= BNB_TEXTURE_2D(BNB_SAMPLER_2D(glfx_L_EYE_CORNEOSCLERA_MASK), var_uv ).x;
	if(toggle.x == 1.){
		bnb_FragColor = c;
