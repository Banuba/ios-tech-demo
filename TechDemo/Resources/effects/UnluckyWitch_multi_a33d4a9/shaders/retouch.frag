...
BNB_IN(3) vec2 glfx_OCCLUSION_UV;
#endif
...
void main()
{
   ...
   vec2 uvh = vec2(abs(2.0 * (var_uv_bg_uv.x - 0.5)),var_uv_bg_uv.y);
    ...
    vec4 makeup2;
        ...
        // makeup2 = BNB_TEXTURE_2D(BNB_SAMPLER_2D(tex_normalMakeup), uvh );
   
        makeup2 = BNB_TEXTURE_2D(BNB_SAMPLER_2D(tex_normalMakeup), var_uv_bg_uv.xy );
    // }
