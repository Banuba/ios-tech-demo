...
BNB_IN(0) vec2 var_cr;

#define texFetchMask(s,t,l) BNB_TEXTURE_2D(s,(vec2(t)+0.5)*inv_sz)
...
mat3 find_mask_coords( BNB_DECLARE_SAMPLER_2D_ARGUMENT(eye_mask), int left_right )
{
    ...
    mat3 prev_et = transpose(mat3(
        BNB_TEXTURE_2D(BNB_SAMPLER_2D(eye_mask_t_prev),vec2(float(left_right),0.)).xyz,
        BNB_TEXTURE_2D(BNB_SAMPLER_2D(eye_mask_t_prev),vec2(float(left_right),1.)).yxz, // swapped x and y just like in output
        vec3(0.,0.,1.) ));

    float alpha = 0.3;
    
    vec2 c_pts = BNB_TEXTURE_2D(BNB_SAMPLER_2D(cmass),vec2(float(left_right),0.)).xy;
