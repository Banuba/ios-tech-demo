...
#define skinSoftIntensity 0.35

#define TEETH_WHITENING
#define teethSharpenIntensity 0.15
#define teethWhiteningCoeff 0.4

#define SHARPEN_EYES
#define EYES_WHITENING
#define eyesSharpenIntensity 0.15
...
BNB_DECLARE_SAMPLER_2D(6, 7, lookupTexTeeth);
...
BNB_DECLARE_SAMPLER_2D(4, 5, tex_makeup);
...
void main()
{
    ...
    res.xyz = softlight_blend_1ch( res.xyz, BNB_TEXTURE_2D(BNB_SAMPLER_2D(tex_makeup), var_uv ).xyz, 0.15 );
   // vec4 shadow = texture( tex_makeup2, var_uv );
   // vec3 face_with_shadow = mix(res.xyz, res.xyz * shadow.xyz, shadow.a);
    
   // bnb_FragColor = vec4(face_with_shadow , 1.0);
    bnb_FragColor = vec4(res.xyz, 1.0);
